...
public class ClassroomService {
    ...
    public void deleteClassroom(Integer classroomId)
    {
        ...
        if(!exists)
        {
            throw new IllegalStateException("classroom with id = "+classroomId+ " does not exist in database");
        }
    ...
    public void updateClassroom(Integer classroomId, String name, Integer facultyId)
    {
        ...
        if(name != null && name.length() > 0 && classroom.getName() != name) //if the new name has been provided is not the same name in database
        {
            ...
            {
                throw new IllegalStateException("Name "+name+" already exist in database");
            }
            classroom.setName(name);
        }
        if(facultyId != null && classroom.getFaculty().getId() != facultyId) //if the new name has been provided is not the same name in database
        {
            classroom.setFaculty(facultyRepository.getById(facultyId));
        }
    }
